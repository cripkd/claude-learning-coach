...
const cheatsheetToggle = el('cheatsheetToggle');
const cheatsheetList = el('cheatsheetList');
...
// Model picker — a plain client preference (not per-course), threaded into
// every /api/chat call. Empty value = "Default" = omit the field entirely
// and let the CLI pick, same as today's behavior before this existed.
const MODEL_KEY = 'coach.model';
try { modelSel.value = localStorage.getItem(MODEL_KEY) || ''; } catch { /* private window, etc. */ }
modelSel.addEventListener('change', () => {
  try { localStorage.setItem(MODEL_KEY, modelSel.value); } catch { /* private window, etc. */ }
});
...
function addBubble(role, text = '') {
  ...
  b.className = 'bubble';
  if (role === 'coach') b.innerHTML = renderMarkdown(text);
...
function selectCourse(slug, hasDashboard) {
  ...
  if (slug === NEW) {
    ...
    cheatsheetToggle.disabled = true;
    closeCheatsheetMenu();
...
// ─── Cheatsheet ─────────────────────────────────────────────────────────────────
// courses/{slug}/cheatsheet.md, rendered standalone (cheatsheet.html) via the
// same markdown.js renderer the chat uses — opened in a new tab or straight
// to print, picked from this dropdown.
function closeCheatsheetMenu() {
  cheatsheetList.hidden = true;
  cheatsheetToggle.setAttribute('aria-expanded', 'false');
}

cheatsheetToggle.addEventListener('click', (e) => {
  e.stopPropagation();
  if (cheatsheetToggle.disabled) return;
  const willOpen = cheatsheetList.hidden;
  closeCheatsheetMenu();
  if (willOpen) { cheatsheetList.hidden = false; cheatsheetToggle.setAttribute('aria-expanded', 'true'); }
});

cheatsheetList.addEventListener('click', (e) => {
  const btn = e.target.closest('button[data-action]');
  closeCheatsheetMenu();
  if (!btn || !currentSlug || currentSlug === NEW) return;
  const url = `/public/cheatsheet.html?slug=${encodeURIComponent(currentSlug)}`
    + (btn.dataset.action === 'print' ? '&print=1' : '');
  window.open(url, '_blank', 'noopener');
});

document.addEventListener('click', (e) => {
  if (!cheatsheetList.hidden && !e.target.closest('#cheatsheetMenu')) closeCheatsheetMenu();
});
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && !cheatsheetList.hidden) closeCheatsheetMenu();
});
...
// esc()/inlineMd()/renderMarkdown() live in markdown.js (shared with
// cheatsheet.html), loaded before this file — see index.html.
...
async function send(message) {
  ...
  const res = await fetch('/api/chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ slug: currentSlug, message, model: modelSel.value || undefined }),
  });
