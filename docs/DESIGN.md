...
## 10. Source-grounding gate (Stop hook + day-start marker)

**The choice:** Daily delivery is fronted by a `Stop` hook (`scripts/day-delivery-gate.mjs`) that refuses to end the assistant turn if the coach claims to deliver Day N (by emitting `<!-- coach:day-start course=<slug> day=N -->` as the first line of the message) but hasn't actually read the source ranges that day requires. Required ranges per day are listed in `sources/_dayplan.md`, generated by `/index-sources` from `phases[].days[].topics` × `_index.md` citations. The checker (`scripts/check-day-readiness.mjs`) extracts every `Read` tool call from the current session's transcript and computes per-range coverage; <90% of any required range fails the gate.

**Rejected alternatives:**
- **Soft CLAUDE.md rule only.** "You MUST read every cited range" already existed in the source-grounding protocol. It failed on the very first delivery — the coach (this one) read 2 of 10 ranges and improvised the rest from training-data knowledge. A rule the coach has to remember is a rule the coach will skip when it's confident.
- **Per-Read instrumentation (`PostToolUse` hook on `Read`).** Original design — log every Read to a session file. Rejected because the transcript already contains every Read tool call with its inputs; building a parallel log was duplicate state. The Stop hook reads the transcript at gate time.
- **Heuristic day-shape detection without a marker.** Block any message that *looks like* a Day delivery (regex on `# Day N` heading). Rejected for now: high false-positive risk (prose mentions of days, recaps, "Day 5 was rough"), and the marker requirement is cheap discipline. May be added later if the coach is observed forgetting the marker in practice.

**Why a marker rather than auto-detection:** The marker is an explicit claim by the coach: "this message delivers Day N." That makes the gate's contract narrow and unambiguous — the gate verifies a specific claim, doesn't guess intent. Prose mentions of a day, recaps, partial-day resumes at quiz stage, watchlist drills, and phase exams all legitimately mention a day number without delivering it; the marker distinguishes them.

**Why coverage and not "did you Read this file":** A coach can Read 5 lines of a 300-line cited range and trivially pass a presence check. Coverage (union of read windows ∩ cited range ≥ 90% of cited span) makes the cost of evasion roughly equal to the cost of compliance.

**Why a sibling `_dayplan.md` rather than re-using `_index.md`:** `_index.md` maps task statements to ranges. A multi-day task statement (e.g., `D1-T1.1` covers Days 1 and 2) has no per-day breakdown there. `_dayplan.md` resolves that — by default it lists the same ranges for both days (forces full reading on Day 1, idempotent on Day 2), and the coach can hand-edit to split coverage when a day genuinely subsets the task.

**Secondary audit (state-write hook):** when `state.json` flips `phases[].days[N].status` from `pending` to `in-progress`/`partial`/`complete`, `scripts/state-write-hook.mjs` scans the current session's transcript for the marker for that day. If absent, it logs a stderr `AUDIT WARNING`. This catches the "marked Day N complete without ever delivering it with the marker" case the Stop hook can't see (the Stop hook only runs when a marker is present). The audit doesn't block — `PostToolUse` fires after the write — but the stderr line is visible in the Claude Code session. Days with no required reading (phase exams, case practice, simulation, endgame) are skipped.

**What the gate explicitly does not catch:**
- Reading via `Bash cat`/`head`/`sed` instead of the `Read` tool — bypasses the gate, would show 0% coverage; deliberately not detected because the patterns are too varied to match reliably. The fix is the existing CLAUDE.md guidance to prefer `Read` for file reads.
- Reading the cited ranges in a *prior* session and recalling them now — current-session scope is deliberate: forces context re-loading at the start of each new session.
- A coach that simply omits the marker — the gate has no signal to fire on. Mitigated by (a) the audit hook, (b) the fact that the gate's existence creates a positive incentive to emit the marker so the readiness check runs and confirms grounding.
